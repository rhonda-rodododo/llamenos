...
3. **I-3 Safari `RTCRtpScriptTransform` → explicit modal with browser recommendation (Rhonda confirmed 2026-04-10).** Spec §5.8 rewritten: when `isSFrameSupported()` returns false and policy is `preferred`, an `E2eeFallbackModal` shows with explicit two-button choice ("Switch browsers and reconnect later" / "Proceed without E2EE this one time"). Copy recommends Firefox (strongest privacy), Brave (stricter Chromium), Chrome/Edge 2025+, Safari 15.4+. `non_e2ee_call_accepted` signed audit entry on proceed. All 22 locales. Rationale: silent fallback is the "false sense of security" anti-pattern from master §3.11; volunteers must actively consent to a non-E2EE path.
4. **I-2 sim-SIP-bridge scope → separate prerequisite PR (Rhonda confirmed 2026-04-10 "reduce context fatigue").** Spec §5.12 updated: the sim-bridge + sim-caller fixtures ship as a dedicated prerequisite PR against main, BEFORE the main Tier 5 PR opens. Fixtures are pure test infrastructure (no SFrame production code). The Tier 5 PR depends on the prerequisite landing first; plan prerequisites block is updated. Fixtures become reusable by Tier 3 + Tier 4 call-path tests.
