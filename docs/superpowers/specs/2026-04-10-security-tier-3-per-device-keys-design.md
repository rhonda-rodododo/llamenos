...
#### 3.3.3. Storage is strictly linear; cross-references are forward-only

**Storage invariant (load-bearing):** every row in the `audit_log` table has **exactly one** `prev_entry_hash` predecessor. The table is a linear hash-chain per `hub_id`, identical in shape to Tier 0's sigchain — Tier 3 does NOT change the storage topology. Concurrent device writes are serialized server-side via optimistic retry: the server inserts with `prev_entry_hash = $expected_head`, returns a 409 if another device wrote between the client's read and the insert, and the client re-reads head + rebuilds + retries. This is the same protocol Git uses for pushes; bounded concurrent writes on the same user sigchain are extremely rare in practice (at most a few per month per user), so the retry cost is negligible.

**Semantic DAG framing.** At a conceptual level, the user sigchain (device adds/revokes, PUK rotations, master key events) and each hub sigchain (membership changes, PTK rotations, hub_key rewraps) form a directed acyclic graph: a hub rotation triggered by a device revoke references the user-sigchain `device_revoke` entry by its `entryHash`, and the verifier resolves those references when reasoning about causality. This is a **read-time** property of the verifier, not a storage property. The canonical hash form in `computeEntryHash()` deliberately does NOT include any array of "parent entry hashes" — there is exactly one predecessor, the hash-linked previous entry in the same chain, and all cross-references are *forward-only pointers* embedded in the typed payload (e.g. a `hub_ptk_rotate` entry's `deviceEnvelopeCommitments` field carries device ids, which the verifier cross-checks against the user sigchain).

**Implementer warning.** Do NOT extend `SignedAuditEntry` to hold a `prevEntryHashes: string[]` array or any other multi-parent field. The DAG framing is purely verifier-side reasoning over forward references in payloads. A storage-level multi-parent design would (a) break compatibility with Tier 0's linear chain verifier, (b) force a merge reconciliation protocol at write time, (c) make `computeEntryHash()` non-canonical (array ordering would matter). Linear storage + forward-reference cross-chain lookup is the correct pattern.
