...
**Fallback UX — decided 2026-04-10: transparent.** When the redirect-flow fallback kicks in, the volunteer sees a brief page transition (the browser navigates to `api.llamenos.example/api/auth/refresh?return_to=...` and immediately back). No modal, no "your browser blocks cookies — click here" dialog, no friction. The transition is ~200–400 ms on a healthy network and is indistinguishable from any other cross-page navigation. The client's auth state layer handles the transition silently: detects the `grant=...` query param on return, exchanges it for a fresh session, removes the query param from the URL with `history.replaceState()`, and continues as if nothing happened. A dev-mode console log records that the fallback path was taken for debugging, but in production nothing is logged on the client side (per `feedback_createDebugLog_stripped_in_prod`). Explicit affordance was considered and rejected: the volunteer does not need to know which cookie policy their browser enforces, and asking them adds friction to the happy path for browsers where the fallback is a no-op. If the redirect-flow itself fails (e.g. the API host is unreachable), the client surfaces a standard "Your session expired — please sign in again" prompt. That is the only user-visible failure mode.
...
**Multiple verifiers — decided 2026-04-10: Tier 4 launch requires ≥ 2 allied-org verifiers, not one.** A single verifier is a single point of silence — compromise of one allied org's GitHub account + silent removal of the verify-llamenos workflow = no verdicts published, and the fleet has no out-of-band signal. Two or more verifiers, run by distinct organisations with distinct Nostr signing keys, mean that silent removal of one is noticed by the other (their Nostr verdict streams no longer agree).

A recipe in `docs/security/VERIFIER_RUNBOOK.md` lets any allied org stand up its own verifier with five commands. More verifiers = more independent attestations. The recipe covers repo setup, cron frequency, Nostr key generation, secret rotation.

**Tier 4 success gate: at least two verifier repositories exist, hosted by two distinct allied organisations, each publishing verdicts to the production Nostr relay, each listed in `src/shared/config/verifiers.ts`** (the committed allowlist of verifier Nostr pubkeys). The gate is verified by `tests/api/releases-manifest.spec.ts` which fetches the current verifier config and asserts `verifiers.length >= 2`. Adding a verifier requires a repo PR — the allowlist is in version control, not a dynamic server-side list, so swapping verifiers silently is an auditable repo change.

**Verifier identity rotation runbook.** Each allied org rotates its verifier Nostr signing key on a schedule (recommended: every 6 months). Rotation procedure: the org generates a new keypair, updates their workflow's `VERIFIER_NSEC` secret, and submits a PR to Llamenos updating `src/shared/config/verifiers.ts` with the new pubkey alongside a "rotated on $date" comment. The old pubkey stays in the allowlist for 7 days to allow in-flight events to verify, then is removed by a follow-up PR. The rotation runbook lives in `docs/security/VERIFIER_RUNBOOK.md`.
...
6. **At least two allied-org verifier repositories exist**, each hosted by a distinct organisation with a distinct Nostr signing key, each running a working GitHub Action that publishes verdicts to the production Nostr relay, and each listed in `src/shared/config/verifiers.ts`. Verified by `tests/api/releases-manifest.spec.ts` which fetches the config and asserts `verifiers.length >= 2`. Manual smoke check at release time also confirms that both verifiers' most recent runs are `success`.
