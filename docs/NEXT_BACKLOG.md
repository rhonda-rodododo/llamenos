...
- [x] ~~**Tier 6 PR #2 — MLS message-path wiring** (Tier 6).~~ **DONE** — Shipped as 8 slice PRs (Slices 1–8) in the `h4-mls-pr2` epic, merged 2026-04-21. PRs: #164 (Slice 2), #165 (Slice 1), #181 (Slice 3), #189 (Slice 4), #193 (Slice 8), #194 (Slice 6), #195 (Slice 5), #208 (Slice 7). Notes and messages now use MLS groupwise encryption; `encryptNote`/`encryptMessage` deleted from `crypto-envelopes.ts`.
