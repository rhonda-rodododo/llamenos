...
## Follow-up: Use GHCR images in production compose (PR #67 follow-up)
- **What:** Once PR #67 merges and the first real release publishes `ghcr.io/rhonda-rodododo/llamenos-hotline:X.Y.Z` + `ghcr.io/rhonda-rodododo/llamenos-hotline-sip-bridge:X.Y.Z`, self-hosted deployments should pull those signed+attested images instead of rebuilding from source on every `docker compose up`. Today `deploy/docker/docker-compose.yml` has `build:` blocks for both `app` (line 34) and `sip-bridge` (line 275); `docker-compose.production.yml` layers on top of that but doesn't override the build. That means every production deploy triggers a fresh `bun install` + `vite build` on the target host.
- **Why:** Builds from registry are faster, reproducible, signed (cosign keyless), SBOM-bearing, and match exactly what the Trivy gate scanned before publish. Self-hosters don't need a build toolchain or the full source tree on their VPS; they just pull. Also lets `watchtower` (already in compose) auto-update to new releases.
- **Two possible shapes — decide during implementation:**
  1. **Separate file** — create `deploy/docker/docker-compose.from-registry.yml` that overrides `app` and `sip-bridge` with `image: ghcr.io/rhonda-rodododo/llamenos-hotline:${LLAMENOS_VERSION:-latest}` and nulls out the `build:` block. Run with `docker compose -f docker-compose.yml -f docker-compose.production.yml -f docker-compose.from-registry.yml up -d`. Pro: explicit, doesn't disturb dev/CI paths. Con: fourth compose file to reason about.
  2. **Default in production.yml** — move the image ref into `docker-compose.production.yml` itself, make it the default for prod, and keep the local-build path only for development/CI. Pro: fewer files. Con: changes the meaning of `-f docker-compose.production.yml`.
- **Ansible side:** `deploy/ansible/roles/llamenos/` also templates compose files. Whatever shape we pick, the Ansible role should default to the from-registry flow for the demo deployment and allow a `llamenos_build_from_source: false` default override for air-gapped installs.
- **Version pinning:** Default to the `X.Y.Z` semver tag (not `latest`) so operators get deterministic deploys and can explicitly `docker compose pull` to upgrade. `LLAMENOS_VERSION` env var in `.env` or `demo_vars.yml`.
- **One-time manual (blocks this work):** GHCR packages must be flipped to Public in repo settings after PR #67 lands — noted in PR body.
