...
- **Tier 1 HPKE (PR-A, in progress)**: New code must use `hpkeSeal` / `hpkeOpen` from `@shared/hpke-primitives` — HPKE RFC 9180 with `DHKEM(X25519) + HKDF-SHA256 + AES-256-GCM`, wire format `EnvelopeV3 { v: 3, labelId, enc, ct }`. AAD is `buildAad(label, recordId, fieldName)` binding ciphertext to (domain, row, column). The hub key is a non-extractable AES-GCM CryptoKey held by `key-store-v3.ts`; identity X25519 stays as zero-on-lock raw bytes until the runtime ships native `deriveBits` for X25519. The legacy ECIES path in `crypto-worker.ts` is retained as a sidecar for 20+ unmigrated call sites; CI grep guardrails block new callers and silent HPKE→ECIES fallback. **Never** add a new caller of `@noble/ciphers/chacha` or `secp256k1.getSharedSecret`. See `docs/security/HPKE_MIGRATION_NOTES.md`.
