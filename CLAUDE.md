...
- **E2EE notes**: MLS groupwise encryption via `@wireapp/core-crypto`. Each hub has a persistent MLS group (`llamenos:hub:<hubId>`). Notes are encrypted/decrypted through the group's ratchet tree — no per-recipient key wrapping. Epoch advances on membership change provide forward secrecy.
- **E2EE messaging**: MLS groupwise encryption via the same hub MLS group as notes. Inbound webhook messages are server-encrypted (AES-GCM under `LABEL_MESSAGE`), then claimed and MLS-encrypted by the first client to fetch; server plaintext is discarded at the webhook boundary.
...
- **Domain separation**: 76 `LABEL_*` constants in `src/shared/crypto-labels.ts` (68 branded `CryptoLabel`, 47 wire-indexed in `LABEL_REGISTRY`) — NEVER use raw string literals for crypto contexts.
...
- **Crypto Web Worker isolation**: All MLS, HPKE, and legacy ECIES/XChaCha20 operations run in a dedicated Web Worker (`src/client/lib/crypto-worker.ts`) via a typed RPC client (`crypto-worker-client.ts`). MLS core-crypto WASM runs inside the worker; the `CoreCrypto` instance and identity key material live in closures, zeroed on lock.
- **Tier 1 HPKE**: New code must use `hpkeSeal` / `hpkeOpen` from `@shared/hpke-primitives` — HPKE RFC 9180 with `DHKEM(X25519) + HKDF-SHA256 + AES-256-GCM`, wire format `HpkeEnvelope { v: 3, labelId, enc, ct }`. AAD is `buildAad(label, recordId, fieldName)` binding ciphertext to (domain, row, column); hub-key encrypted records whose ids are server-generated UUIDs require the **client to pre-generate the id** and pass it through the create mutation so the server stores the same id used as AAD `recordId` on seal. The HPKE private key and hub AES-GCM key are non-extractable `CryptoKey` handles held inside the crypto-worker closure (installed via `unlockWithHandles`); identity X25519 stays as zero-on-lock raw bytes until the runtime ships native `deriveBits` for X25519. The legacy ECIES path in `crypto-worker.ts` is retained as a sidecar for remaining call sites (blasts, envelope-encrypted PII, backup, provisioning, key-store PIN-KEK, hub-key-wrap, and server-side IdP/event/agent crypto); notes and messages moved to MLS in Tier 6 PR #2. CI grep guardrails block new callers and silent HPKE→ECIES fallback. **Never** add a new caller of `@noble/ciphers/chacha` or `secp256k1.getSharedSecret`. See `docs/security/HPKE_MIGRATION_NOTES.md`.
...
2. **Hub-key encrypted org metadata** (role names, shift names, report type names, custom field labels, team names): AES-256-GCM via non-extractable WebCrypto `CryptoKey` with per-record AAD binding (`hubFieldAad(recordId, fieldName)`). Decrypted client-side via `decryptHubField()`.
3. **MLS groupwise encryption** (call notes, messages): Encrypted/decrypted via the hub's MLS group (`MlsConversation.encrypt()` / `decrypt()`). Epoch advances on membership change provide forward secrecy.
...
- **Release PR merging is human-gated.** Don't merge the release PR autonomously — even when main looks green. A release is a deliberate ship decision the user makes. When the user asks you to cut a release, verify all three of: **(a)** main's tip commit is green on `ci.yml` and every required workflow (`gh run list --branch main --limit 1`) — the release PR itself has NO CI runs because GitHub's anti-recursion rule blocks downstream workflows for GITHUB_TOKEN-authored PRs, so main's tip IS the effective state; **(b)** the release PR is up-to-date with main (`gh pr view <N> --json mergeStateStatus` should report `CLEAN`, not `BEHIND` or `BLOCKED`); **(c)** the `CHANGELOG.md` diff content looks reasonable for a real release. Only then merge. If any of (a)/(b)/(c) is missing, fix the cause or report — never merge on an "almost green" main. No flake excuses, no "the failing check is unrelated" rationalizations.
...
- **Uses `GITHUB_TOKEN` with explicit write permissions** — no repository PAT required. `knope-release-pr.yml` and `release.yml` both grant `contents: write` and `pull-requests: write` at the workflow level. Requires the repo setting **Settings → Actions → General → Workflow permissions → "Allow GitHub Actions to create and approve pull requests"** (verify via `gh api repos/:owner/:repo/actions/permissions/workflow` — `can_approve_pull_request_reviews` must be `true`). Because `GITHUB_TOKEN`-created resources do not fire downstream events, `release.yml` explicitly dispatches `auto-deploy-demo.yml` via `gh workflow run` at the end of a successful release (the `on: release: published` trigger would be skipped).
